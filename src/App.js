...
// --- 1. 智慧字典：自動辨識各校 Excel 不同表頭 ---
const columnDictionary = {
  name: ['課程名稱', '科目', '課名', '課程', '科目名稱', 'course', 'subject'],
  teacher: ['老師', '教師', '授課教師', '教授', '任課教師', 'teacher', 'professor'],
  credit: ['學分', '學分數', 'credit', 'units'],
  category: ['必選修', '修別', '必/選修', '屬性', '選必修', '必選', 'type'],
  time: ['時間', '節次', '星期', '上課時間', '星期/節次', 'time', 'schedule'],
  dept: ['系所', '開課單位', '系級', 'dept', 'department']
};
...
export default function App() {
  ...
  const [courses, setCourses] = useState([]); // 初始設為空，由匯入或預設值填充
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const filteredMapped = rawData
        .map((row, i) => {
          let stdRow = { id: `ex-${Date.now()}-${i}` };
          Object.entries(row).forEach(([key, value]) => {
            const stdKey = getStandardKey(key);
            if (stdKey) {
              stdRow[stdKey] = (stdKey === 'credit') ? Number(value) : String(value).trim();
            }
          });
          return stdRow;
        })
        .filter(c => {
          // ✨ 智慧過濾器：過濾掉 Excel 中的標題列、空行、雜訊
          const isInvalidName = !c.name || c.name.includes("學年") || c.name.includes("學期") || c.name.includes("科目類別") || c.name.length < 2;
          const hasNoCredit = isNaN(c.credit) || c.credit === 0;
          return !isInvalidName && !hasNoCredit;
        })
        .map(c => ({
          ...c,
          name: c.name,
          ...
          credit: c.credit || 0,
          time: c.time || "",
          category: c.category || "選修",
          dept: c.dept || "未知"
        }));
      
      setCourses(prev => [...prev, ...filteredMapped]);
      alert(`匯入成功！已自動過濾雜訊，新增 ${filteredMapped.length} 門有效課程。`);
    };
    reader.readAsBinaryString(file);
  };

  // --- 3. 時間解析：支援「一34」、「(一)34」、「週一 3-4」 ---
  ...
  const { table, conflicts, currentCredits, requiredCredits } = useMemo(() => {
    ...
    schedule.forEach((course) => {
      ...
      if (course.category.includes("必")) reqCredits += course.credit;
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-7 rounded-[2.5rem] border border-purple-50 shadow-sm">
            ...
            <p className="text-xs text-slate-400 mt-2 font-medium">尚缺 {GRAD_REQUIRED - currentCredits} 學分</p>
          </div>
          <div className={`p-7 rounded-[2.5rem] border shadow-sm ${conflicts.length > 0 ? "bg-rose-50 border-rose-100" : "bg-emerald-50 border-emerald-100"}`}>
            ...
            <h3 className={`text-sm font-bold mt-2 ${conflicts.length > 0 ? "text-rose-600" : "text-emerald-600"}`}>{conflicts.length > 0 ? `🛑 偵測到 ${conflicts.length} 處衝堂` : "✅ 課表編排正常"}</h3>
          </div>
        </div>
        ...
        {page === "courses" && (
          <div className="space-y-6">
            <div className="bg-white/50 backdrop-blur-md p-4 rounded-[2rem] border border-purple-50 flex flex-wrap gap-3 items-center shadow-sm">
              <input type="text" placeholder="搜尋課程..." className="flex-1 p-3 rounded-2xl bg-white border border-slate-100 outline-none text-sm focus:ring-2 focus:ring-purple-200" onChange={e => setSearch(e.target.value)} />
              <label className="cursor-pointer bg-slate-900 text-white px-6 py-3 rounded-2xl font-bold text-xs hover:bg-purple-600 transition shadow-lg">
                📁 匯入 Excel <input type="file" onChange={handleFile} className="hidden" accept=".xlsx, .xls" />
              </label>
            </div>
              ...
              {courses.filter(c => c.name.includes(search)).map(course => (
                <div key={course.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
                  <div className="flex justify-between items-start mb-4">
                    <span className={`px-3 py-1 text-[9px] font-black rounded-lg uppercase ${course.category.includes('必') ? 'bg-purple-100 text-purple-600' : 'bg-slate-100 text-slate-400'}`}>{course.category}</span>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => addCourse(course)} className="w-10 h-10 bg-purple-50 text-purple-600 rounded-full flex items-center justify-center hover:bg-purple-600 hover:text-white transition shadow-sm font-bold">＋</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div ref={componentRef} className="bg-white p-6 md:p-10 rounded-[3rem] shadow-2xl border border-purple-50">
             <div className="flex justify-between items-center mb-10">
                ...
                <div className="flex gap-2">
                   <button onClick={() => setSchedule([])} className="px-4 py-2 bg-rose-50 text-rose-500 rounded-xl text-xs font-bold no-print">清空</button>
                   <button onClick={handlePrint} className="px-4 py-2 bg-purple-600 text-white rounded-xl text-xs font-bold no-print">匯出 PDF</button>
                </div>
             </div>
                      ...
                      <tr key={i}>
                        ...
                        {row.slice(1).map((cell, j) => (
                          <td key={j} className={`p-1.5 h-24 rounded-[1.8rem] border ${cell.length > 1 ? 'bg-rose-50 border-rose-100' : 'bg-slate-50/40 border-slate-100/50'}`}>
                            {cell.map(c => (
                              <div key={c.id} className={`p-2.5 mb-1.5 rounded-2xl text-[10px] font-black shadow-sm border ${c.category.includes('必') ? 'bg-purple-600 text-white' : 'bg-white text-purple-600 border-purple-100'}`}>
                                {c.name}
                              </div>
                            ))}
                          </td>
